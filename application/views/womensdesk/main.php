...
<div id="content-container">
	...
	<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
	<div id="page-title">
		<h1 class="page-header text-overflow">Women's Desk</h1>
	</div>
	<?php
	if($success){
					...
					<div class="pad-btm form-inline">
						<div class="row">
							<div class="col-sm-6 table-toolbar-left">
								<a href="<?php echo base_url('citizens/add/womensdesk/add'); ?>" class="btn btn-success btn-labeled fa fa-plus">New Citizen</a>
							</div>
						</div>
					</div>
				</div>
								...
								<?php foreach($citizens as $row): ?>
									<tr>
										...
										<td style="text-align: center; vertical-align: middle;">
											<a href="<?php echo base_url('womensdesk/add/'.$row->Citizen_ID) ?>" class="btn btn-primary">Select</a>
										</td>										
									</tr>
								<?php endforeach; ?>
							</tbody>
		...
		<div class="panel">
			<div class="panel-body">
				<div class="pad-btm form-inline">
					<div class="row">
						<div class="col-sm-6 table-toolbar-left">
							<button type="button" data-toggle="modal" data-target="#myModalSearch" class="btn btn-success btn-labeled fa fa-plus">New Apprehension</button>
						</div>
					</div>
				</div>
				<?php if(is_array($womensdesk)): ?>
					<table id="demo-dt-selection" class="table table-responsive table-hover" cellspacing="0" width="100%">
						<thead>
							<tr>
								<th></th>
								<th>Apprehension ID</th>
								<th>Full Name</th>
								<th>OIC</th>
								<th>Date Created</th>
							</tr>
						...
						<tbody>
							<?php foreach($womensdesk as $row): ?>
								<tr>
									<?php echo "<td class='text-center'>" . anchor(base_url().'womensdesk/view/'.$row->WD_ID, '<button class="btn btn-default" title="View Apprehension"><i class="fa fa-search"></i></button>'). "</td>";
									?>
									<td><?php echo $row->Apprehension_ID; ?></td>
									<td><?php echo $row->Citizen_Title.'. '.$row->Citizen_LastName.', '.$row->Citizen_FirstName.' '.$row->Citizen_MiddleName; ?> <?php echo $row->Citizen_Suffix == 'NA'? '':$row->Citizen_Suffix; ?></td>
									<td><?php echo $row->OIC_FullName; ?></td>
